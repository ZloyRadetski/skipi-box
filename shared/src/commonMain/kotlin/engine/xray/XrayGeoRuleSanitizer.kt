...
object XrayGeoRuleSanitizer {
    ...
    private fun checkTagInDatFile(dataDir: String, fileName: String, tag: String, originalRule: String): Boolean {
        ...
        if (!datFile.isFile || datFile.length() <= 0) {
            AppLogger.warn(LogTag, "Resource file '$fileName' not found or empty in '$dataDir', skipping rule '$originalRule' to prevent Xray crash")
            return false
        }
        val tags = GeoDatParser.parseTags(datFile)
        val exists = tags.any { it.equals(tag, ignoreCase = true) }
        if (!exists) {
            AppLogger.warn(LogTag, "Geo tag '$tag' not found in '$fileName', skipping rule '$originalRule' to prevent Xray crash")
            return false
        }
        return true
    }
