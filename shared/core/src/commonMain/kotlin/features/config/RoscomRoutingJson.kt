...
/** A geo-resource preset available to a platform importing a routing profile. */
data class RoscomRoutingResourceSource(
    val id: Int,
    val geoIpUrl: String,
 ...
 * regular Shadowrocket profile so platform import code can take over.
 */
...
fun String.toRoscomRoutingJsonOrNull(): JsonObject? {
    val trimmed = trim()
 ...
 *
 * Resource source IDs stay platform-defined. This makes the converter usable
 * by Android and Desktop without encoding Android preferences in shared code.
 */
fun JsonObject.toRoscomRoutingShadowrocketConf(
    fallbackName: String,
    resourceSources: List<RoscomRoutingResourceSource>,
    customResourceSourceId: Int,
): String {
    ...
    val skipiLines = buildList {
        ...
        if (geoIpUrl.isNotEmpty() || geositeUrl.isNotEmpty()) {
            ...
            // matching preset when possible and fall back to the caller's
            // portable custom-source ID otherwise.
            val preset = resourceSources.firstOrNull { source ->
                (geoIpUrl.isEmpty() || source.geoIpUrl == geoIpUrl) &&
                    (geositeUrl.isEmpty() || source.geoSiteUrl == geositeUrl)
            }
            add("resource-source = ${preset?.id ?: customResourceSourceId}")
