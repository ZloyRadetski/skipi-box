...
sealed interface WireguardConfParseResult {
    object NotWireguardConf : WireguardConfParseResult
...
/**
 * Parses a standard WireGuard or AmneziaWG `.conf` without filesystem, UI or
 * Android dependencies. The caller decides how an invalid document is shown
 * or logged.
 */
fun parseWireguardConf(text: String): WireguardConfParseResult {
    ...
    if (!clean.contains("[Interface]", ignoreCase = true) ||
        ...
        return WireguardConfParseResult.NotWireguardConf
    }
    ...
    return runCatching {
        ...
        // AmneziaWG obfuscation fields live in [Interface].
        ...
        WireguardConfParseResult.Imported(serverObj)
    }.getOrElse { error ->
        WireguardConfParseResult.Invalid(error)
    }
}
