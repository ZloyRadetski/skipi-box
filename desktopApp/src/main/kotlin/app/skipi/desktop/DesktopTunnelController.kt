...
class DesktopTunnelController(
    ...
    override suspend fun snapshot(): TunnelSnapshot = synchronized(this) {
        if (latestSnapshot.phase == TunnelPhase.Connected && !processState().isRunning) {
            latestSnapshot = failedSnapshot(IllegalStateException("Xray process stopped unexpectedly"))
        }
        latestSnapshot
    }

    override fun supports(capability: TunnelCapability): Boolean = false
